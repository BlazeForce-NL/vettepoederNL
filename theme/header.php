...
<header class="site-header">
  <div class="wrap nav">
    <a class="brand" href="<?php echo esc_url(home_url('/')); ?>" aria-label="<?php echo esc_attr(get_bloginfo('name')); ?> home">
      <span class="brand-mark">VP</span><span><?php bloginfo('name'); ?></span>
    </a>
    <button class="pf-menu-toggle" type="button" data-menu-toggle aria-controls="site-navigation" aria-expanded="false"><span aria-hidden="true">☰</span><span>Menu</span></button>
    <nav id="site-navigation" aria-label="Hoofdnavigatie">
      <?php if (function_exists('bartdoggen_menu')) { bartdoggen_menu(); } else { site_menu(); } ?>
    </nav>
    <div class="site-language-switcher" aria-label="Taalkeuze"><?php site_language_switcher(); ?></div>
  </div>
</header>
